...
fac_name = []
rountTimedelay = []
for faculty in testtime.myresult:
    txtnum = faculty[0]
    sumnum += txtnum
    txtfacname = faculty[1]
    num_of_graduates.append(faculty[0])
    fac_name.append(faculty[1])
    txt += "[ "+txtfacname+" จำนวน "+str(txtnum)+" คน]"

txt += " จำนวนนักศึกษาช่วงเช้าทั้งสิ้น "+str(sumnum) + " คน"
msg = txt
r = requests.post(url, headers=headers, data={'message': msg})
msg = "## เริ่มการนับ การปรับปรุงสถานะทุกๆ 30(งานจริง 180 วินาที) วินาที"
r = requests.post(url, headers=headers, data={'message': msg})
...
modelWeights = "C:/OPENCVP1/yolofile/custom-yolov4-detector_best_touch.weights"
        ...
        # store the trackable object in our dictionary
            ...
            if (total >= 1 and   checklen <= len(fac_name) ):
                ...
                if delta.seconds >= 30:
                    ...
                    txt += "เฉลี่ยคนละ : " + \
                        str(x) + " วินาทีหรือ [จำนวนคน] คน/นาที เวลาปัจจุบัน " + \
                        current_time+"คาดการณ์เวลาสิ้นสุด [ยังไม่ได้คำนวณ]"
                    # msg = txt
                    # r = requests.post(url, headers=headers,
                    #                   data={'message': msg})
                    ...
                    print("เฉลี่ยคนละ : ", avtime,
                          " วินาทีหรือ [จำนวนคน] คน/นาที เวลาปัจจุบัน "+current_time+"คาดการณ์เวลาสิ้นสุด [ยังไม่ได้คำนวณ]")
